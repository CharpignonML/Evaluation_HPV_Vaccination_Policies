...
df <- read.csv2("/Users/charlottederogis/Desktop/Stage/R/discontinuités/googletrends/16122019.csv")
df$Date <- as.Date(df$Date)
df$n <- as.numeric(df$n)
t0 <- as.Date("2019-12-16")

# 3 semaines avant et après
df_3 <- df %>%
  filter(Date >= t0 - 21, Date <= t0 + 21)

mediane_avant <- median(df_3$n[df_3$Date < t0], na.rm = TRUE)

# Préparation des données pour le fit 

post_3 <- df_3[df_3$Date >= as.Date("2019-12-16"), ]
post_3$t          <- post_3$Date -  as.Date("2019-12-16")
post_3$n_centré <- post_3$n - mediane_avant

post_3$t <- as.numeric(post_3$t)
...
SS_tot        <- sum((post_3$n_centré - mean(post_3$n_centré))^2)
R2            <- 1 - (SS_res / SS_tot)
...
t_grid <- seq(0, max(post_3$t), length.out = 200)
df_fit <- data.frame(
  Date = t_grid + t0,
  n    = mediane_avant + predict(fit_weibull, newdata = data.frame(t = t_grid))
)

ggplot(df_3, aes(x = Date, y = n)) +
  ...
  annotate("text", x = min(df_3$Date), y = mediane_avant + 3,
           label = paste("Médiane avant =", round(mediane_avant, 1)),
           hjust = 0, color = "gray40", size = 5) +
  annotate("text", x = t0 - 1, y =  max(df_3$n),
           label = paste("16 decembre 2019"),
