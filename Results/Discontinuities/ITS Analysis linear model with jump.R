...
# Step design: change in level AND change in slope around time zero
m_step <- lm(n ~ t * treated, data = df)

# Fitted values
df$fitted_m_step <- m_step$fitted.values

# Transform the dataset into a long format
df_long <- df %>% 
  pivot_longer(cols = c(n, fitted_m_step), 
               names_to = "model", 
               values_to = "fitted_value")
df_long$model_f <- factor(df_long$model,
                              levels=c("n","fitted_m_step"))

# Model summary

# Formatting, using x decimal digits (by default: x=2)
number_decimal_digits <- 2
msummary(list('Coefficients' = m_step),
         stars = c('*' = .1, '**' = .05, '***' = .01), 
         vcov = 'robust',
...
pre_policy_position_y <- 75000

post_policy_position_x <- 1.5
post_policy_position_y <- 75000

policy_font_size <- 4.5
policy_color <- "black"

time_zero_position_x <- -0.15
time_zero_hjust <- 1.1
time_zero_font_size <- 4
time_zero_color <- "black"

legend_position <- c(0.2,0.86)
...
y_axis_limits <- c(NA, NA)

# Figure
p <- ggplot(df_hsh_long, aes(x = t/365.25, y = fitted_value, group = model_f)) +
  ...
  # Legend labels
  scale_color_manual(
    values = c("fitted_m_step" = "#F8766D", "n" = "#00BFC4"),
    labels = c("n" = "Observed", "fitted_m_step" = "Fitted_m_linear_jump")
  ) +
  ...
  annotate("text", x = post_policy_position_x, y = post_policy_position_y, label = "Post-policy",
           colour = policy_color, fontface='italic', size = policy_font_size) +
  annotate("text", x = time_zero_position_x, y = Inf, label = "Time zero",
           colour = time_zero_color, size = time_zero_font_size, angle = 90,
           hjust = time_zero_hjust) +
